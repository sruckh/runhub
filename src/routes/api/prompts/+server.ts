...
export const POST: RequestHandler = async ({ request }) => {
  try {
    const { provider, subject, geminiKey, runpodKey } = await request.json();
...
Output ONLY the prompt text, nothing else.`,
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert image prompt engineer. Generate a detailed, high-quality prompt for an image generation model based on the user's request. Output ONLY the final prompt.",
            },
            {
              role: "user",
              content: subject,
      ...
      if (!response.ok) {
        ...
        return json(
          { error: `RunPod API error: ${response.status}`, details: errorText },
      ...
      if (data.status === "COMPLETED" && data.output && data.output.result) {
        ...
        if (choices && choices.length > 0) {
          ...
          const cleanContent = content
            .replace(/^(PROMPT:|\*\*AI Image Prompt:\*\*)\s*/i, "")
            .trim();
